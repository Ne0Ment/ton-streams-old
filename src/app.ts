import { Bot, InlineKeyboard, Context, SessionFlavor, session, GrammyError } from "grammy";
...
async function CloseContract(contract) {
    try {
        ...
        await contract.fromWalletV.close({
            ...contract.currentState,
            hisSignature: signatureCloseS
        }).send(toNano('0.05'));
    } catch (error) {
        console.log(error);
    }
...
async function initWallet(apiKey, seedS, seedV, fullPrice, initId) {
    try {
        ...
        const channelS = tonweb.payments.createChannel({ ...channelConfig, isA: false, myKeyPair: keyPairS, hisPublicKey: keyPairV.publicKey });
        const channelV = tonweb.payments.createChannel({ ...channelConfig, isA: true, myKeyPair: keyPairV, hisPublicKey: keyPairS.publicKey });
...
function initialCreationData(): CreationData {
    return {
        channelId: -1,
        minutesLength: -1, //length in minutes
        price: -1
    }
}
...
function createCallback(action: string, data = {}) {
...
function unix2string(timestamp: number) {
...
bot.on('chat_join_request', async (ctx) => {
    try {
        connection.execute('SELECT * from subscriptions WHERE userId = ? AND channelID = ? And active = ? ORDER BY id DESC', [ctx.chatJoinRequest.from.id, ctx.chatJoinRequest.chat.id, true],
    ...
    try {
        ...
        for (const admin of admins.map(t => t.user.id))
            ctx.api.sendMessage(admin, `Обнаружена запланированная трансляция в канале "${ctx.channelPost.chat['title']}"`, {
                reply_markup: new InlineKeyboard().text('Настроить трансляцию', createCallback('setup_stream', { channelId: ctx.chat.id }))
            });
    } catch (error) {
        console.log(error)
    }
...
async function watchStream(channelId: number, streamId: number, pricePerCheck: number) {
    connection.execute('SELECT * FROM subscriptions WHERE streamId = ? AND active = ? ORDER BY id DESC', [streamId, true],
        async (err, streamSubs, fields) => {
...
bot.on('channel_post:video_chat_started', async (ctx) => {
    try {
        connection.execute('SELECT * FROM streams WHERE channelId = ? AND ended = ? ORDER BY id DESC', [ctx.channelPost.chat.id, false],
            async (err, results, fields) => {
                if (results.length != 0) {
                    const foundStream = results[0];
                    connection.query('UPDATE streams SET started = ? WHERE id = ?', [true, foundStream.streamId],
                        async (err, results, fields) => { });
                    connection.execute('SELECT * FROM subscriptions WHERE channelId = ? AND active = ? ORDER BY id DESC', [ctx.channelPost.chat.id, true],
                        async (err, subs, fields) => {
            ...
            async (err, results, fields) => {
                if (results) {
                    if (results.length != 0) {
                        ...
                        setTimeout(async () => {
                            connection.query('UPDATE subscriptions SET active = ? WHERE channelId = ?', [false, ctx.channelPost.chat.id],
                                async (err, results, fields) => { });
                            connection.query('UPDATE streams SET ended = ? WHERE channelId = ?', [true, ctx.channelPost.chat.id],
                                async (err, results, fields) => { })
                        }, 200)

                        clearInterval(streamWatchers[results[0].streamId]);
                    }
                }
            })
    } catch (error){
...
bot.command(['start', 'menu'], async (ctx) => {
    try {
        ...
        if (splitted.length > 1) {
            ...
            connection.execute('SELECT * FROM streams WHERE id = ? ORDER BY id DESC', [streamId],
                async (err, results, fields) => {
                    if (results.length != 0) {
                        ...
                        await ctx.reply(`Оплата трансляции\nДата проведения: ${unix2string(stream.scheduledDate)}\nЦена: ${stream.price}TON`, {
                            reply_markup: new InlineKeyboard().text('Оплатить', createCallback('initiate_payment', { streamId: streamId })).row()
                        });
                    } else {
        ...
        if (ctx.session.messageStatus == 'setup_stream') {
            if (ctx.session.creationData.minutesLength < 0) {
                ...
                if (parsedPrice > 0) {
                    ...
                    let createdStream: Stream = creatingStreams[ctx.session.creationData.channelId];
                    ...
                    connection.query('INSERT INTO streams SET ?', createdStream,
                        async (err, results, fields) => {
                            const botLink = `https://t.me/${botname}?start=${results.insertId}`;
                            connection.query('UPDATE streams SET botLink = ?', [botLink],
                                async (err, results, fields) => {
            ...
            connection.execute(
                'SELECT FROM users WHERE userId = ?', [ctx.message.chat.id],
                (err, results, fields) => {
                    if (results) {
                        connection.query('UPDATE users SET seedPhrase = ? WHERE userId = ?', [ctx.message.text, ctx.message.chat.id],
                            (err, results, fields) => {
                                complete();
                            })
                        userId = results[0]
                    } else {
                        connection.query('INSERT INTO users SET ?', { seedPhrase: ctx.message.text, userId: ctx.message.chat.id },
                            (err, results, fields) => {
                                complete();
                            })
                    }
...
bot.on('callback_query:data', async (ctx) => {
    try {
        ...
        if (action == 'main_menu') {
            ...
            connection.execute('SELECT * FROM streams WHERE creatorId = ? AND ended = ? ORDER BY id DESC', [ctx.callbackQuery.message.chat.id, false],
                async (err, results, fields) => {
                    let createdStreamsKeyboard = new InlineKeyboard();
                    for (const stream of results) {
                        createdStreamsKeyboard.text(`${stream.channelName} - ${unix2string(stream.scheduledDate)}`,
                            createCallback('sd', { id: stream.id, cb: 'c' })).row()
                    }
            ...
            connection.execute('SELECT * FROM streams WHERE id = ? ORDER BY id DESC', [data.id],
                async (err, results, fields) => {
                    const stream = results[0];
                    ...
                    let detailsKeyboard = new InlineKeyboard()
                    if (data.cb == 'b') {
            ...
            connection.execute('SELECT * FROM subscriptions INNER JOIN streams ON subscriptions.streamId=streams.id WHERE userId = ? AND active = ? ORDER BY id DESC', [ctx.callbackQuery.message.chat.id, true],
                async (err, results, fields) => {
                    const userStreams = results;
                    let userStreamsKeyboard = new InlineKeyboard();
                    if (results) {
            ...
            connection.execute('SELECT * FROM streams WHERE id = ? ORDER BY id DESC', [data.streamId],
                async (err, results, fields) => {
                    const stream = results[0];
                    ...
                    connection.execute('SELECT * FROM users WHERE userId = ? ORDER BY id DESC', [ctx.callbackQuery.message.chat.id],
                        async (err, results, fields) => {
                            const seedV = await tonMnemonic.mnemonicToSeed(results[0].seedPhrase.split(' '));
                            connection.execute('SELECT * FROM users WHERE userId = ? ORDER BY id DESC', [stream.creatorId],
                                async (err, results, fields) => {
                                    ...
                                    initWallet(apiKey, seedS, seedV, stream.price, initId).then(contract => {
                                        ...
                                        connection.query('INSERT INTO subscriptions SET ?', subscription,
                                            async (err, results, fields) => {
            ...
            connection.execute('SELECT * FROM subscriptions WHERE streamId = ? AND userId = ? ORDER BY id DESC', [data.id, ctx.callbackQuery.message.chat.id],
                async (err, results, fields) => {
